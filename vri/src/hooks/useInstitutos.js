...
export const useInstitutos = (year = '2025') => {
        ...
        const fetchYears = async () => {
            try {
                const [resF, resR] = await Promise.all([
                    fetch(`${API_URL}/api/institutos-financiados?fields[0]=anio&pagination[limit]=100`),
                    fetch(`${API_URL}/api/institutos-reconocidos?fields[0]=anio&pagination[limit]=100`)
                ]);
                ...
                console.error('Error fetching institutos years:', err);
            }
        };
        fetchYears();
    }, []);
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                const [resF, resR] = await Promise.all([
                    fetch(`${API_URL}/api/institutos-financiados?filters[anio][$eq]=${year}&pagination[limit]=100`),
                    fetch(`${API_URL}/api/institutos-reconocidos?filters[anio][$eq]=${year}&pagination[limit]=100`)
                ]);
                ...
                const mappedF = (jsonF.data || []).map(item => {
                    ...
                    return {
                        ...
                        instituto: data.instituto_siglas || '',
                        area: data.area || '',
                        puntaje: data.puntaje || '',
                        resultado: data.resultado || '',
                        observaciones: (data.observaciones || '').split('\n').filter(s => s.trim() !== '')
                    };
                });
                ...
                console.error('Error fetching institutos data:', err);
            } finally {
                setLoading(false);
            }
        };
